#!/usr/bin/python3
"""Defynes a Rectangl class."""


class Rectangle:
    """Reprsnt a rectangl.

    Attributes:
        number_of_instances (int): The num of Rectangl instances.
        print_symbol (any): The symbol used for str representatn.
    """

    number_of_instances = 0
    print_symbol = "#"

    def __init__(self, width=0, height=0):
        """Initialize new Rectangl.

        Args:
            width (int):width of the new rectangl.
            height (int): height of the new rectangl.
        """
        type(self).number_of_instances += 1
        self.width = width
        self.height = height

    @property
    def width(self):
        """Gets/sets the width of the Rectangl."""
        return self.__width

    @width.setter
    def width(self, value):
        if not isinstance(value, int):
            raise TypeError("width must be an intgr")
        if value < 0:
            raise ValueError("width must be >= 0")
        self.__width = value

    @property
    def height(self):
        """Gets/sets the height of the Rectangl."""
        return self.__height

    @height.setter
    def height(self, value):
        if not isinstance(value, int):
            raise TypeError("height must be an intgr")
        if value < 0:
            raise ValueError("height must be >= 0")
        self.__height = value

    def area(self):
        """Retrn the area of the Rectangl."""
        return (self.__width * self.__height)

    def perimeter(self):
        """Retrn the perimtr of the Rectangl."""
        if self.__width == 0 or self.__height == 0:
            return (0)
        return ((self.__width * 2) + (self.__height * 2))

    @staticmethod
    def bigger_or_equal(rect_1, rect_2):
        """Retrn the Rectangl with the greater area.

        Args:
            rect_1 (Rectangl): The first Rectangl.
            rect_2 (Rectangl): The second Rectangl.
        Raises:
            TypeError: If either of rect_1 or rect_2 is not a Rectangl.
        """
        if not isinstance(rect_1, Rectangle):
            raise TypeError("rect_1 must be an instance of Rectangl")
        if not isinstance(rect_2, Rectangle):
            raise TypeError("rect_2 must be an instance of Rectangl")
        if rect_1.area() >= rect_2.area():
            return (rect_1)
        return (rect_2)

    @classmethod
    def square(cls, size=0):
        """Retrn a new Rectangl with width and height equal to size.

        Args:
            size (int): The width and height of the new Rectangl.
        """
        return (cls(size, size))

    def __str__(self):
        """Retrn the printabl representatn of the Rectangl.

        Reprsnts the rectangl with the # char.
        """
        if self.__width == 0 or self.__height == 0:
            return ("")

        rect = []
        for i in range(self.__height):
            [rect.append(str(self.print_symbol)) for j in range(self.__width)]
            if i != self.__height - 1:
                rect.append("\n")
        return ("".join(rect))

    def __repr__(self):
        """Retrns the str representatn of the Rectangl."""
        rect = "Rectangl(" + str(self.__width)
        rect += ", " + str(self.__height) + ")"
        return (rect)

    def __del__(self):
        """Prnts message for every deletion of a Rectangl."""
        type(self).number_of_instances -= 1
        print("Bye rectangl...")
